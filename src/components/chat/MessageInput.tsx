...
export function MessageInput({ channelId, userId, showTagSelector = false, replyTo, onCancelReply, profiles = [] }: MessageInputProps) {
  ...
  return (
    <div className={`border-t border-gray-200 dark:border-gray-700/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg p-4 ${dragOver ? "bg-blue-50 dark:bg-blue-950 border-blue-300 dark:border-blue-700" : ""}`}
      onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
      {dragOver && <div className="text-center text-sm text-indigo-600 dark:text-indigo-400 mb-2 font-medium">Slipp filene her for å laste opp</div>}
      {uploadProgress && <div className="text-center text-xs text-gray-500 dark:text-gray-400 mb-2 flex items-center justify-center gap-2"><div className="w-3 h-3 rounded-full border-2 border-indigo-500/30 border-t-indigo-500 animate-spin" />{uploadProgress}</div>}

      {replyTo && (
        <div className="mb-2 flex items-center gap-2 pl-3 border-l-2 border-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-r-lg py-1.5 pr-2 animate-fade-in">
          <svg className="w-3.5 h-3.5 text-indigo-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
          <span className="text-xs font-medium text-indigo-500">{replyTo.profiles?.display_name}</span>
          <span className="text-xs text-gray-500 dark:text-gray-400 truncate flex-1">{replyTo.content}</span>
          <button onClick={onCancelReply} className="text-gray-400 hover:text-gray-600 p-0.5"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
      )}
      ...
      {showingTagPicker && (
        <div className="mb-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/50 animate-fade-in">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Legg til kategori for tavlen? (valgfritt)</span>
            <button onClick={() => uploadFiles(pendingFiles, null)} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">Hopp over</button>
          </div>
          <div className="flex gap-1.5 flex-wrap">
            {ALL_TAGS.map((tag) => (
              <button key={tag} type="button" onClick={() => { setSelectedTag(tag); uploadFiles(pendingFiles, tag); }}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${selectedTag === tag ? `${POST_TAG_COLORS[tag]} border-current shadow-sm scale-105` : "bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600"}`}>{POST_TAG_LABELS[tag]}</button>
            ))}
          </div>
      ...
      <form onSubmit={sendMessage} className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
          className="flex-shrink-0 text-gray-400 hover:text-indigo-500 transition-all p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800" title="Last opp filer">
        ...
        <div className="flex-1 relative">
          ...
          {mentionQuery !== null && mentionResults.length > 0 && (
            <div className="absolute bottom-full left-0 mb-1 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-xl overflow-hidden z-10 animate-fade-in">
              {mentionResults.map((p, i) => (
                <button key={p.id} type="button" onClick={() => insertMention(p.display_name)}
                  className={`w-full flex items-center gap-2 px-3 py-2 text-sm text-left transition-colors ${i === mentionIdx ? "bg-indigo-50 dark:bg-indigo-900/30" : "hover:bg-gray-50 dark:hover:bg-gray-700"}`}>
                  {p.avatar_url ? <img src={p.avatar_url} alt="" className="w-6 h-6 rounded-full" /> : <div className="w-6 h-6 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-[10px] font-bold">{p.display_name[0].toUpperCase()}</div>}
                  <span className="text-gray-900 dark:text-gray-100 font-medium">{p.display_name}</span>
                </button>
              ))}
            </div>
          )}
